...
%    criterion R and the size of the neighbourhood S. The defeault is R=15
%    and S=Inf.
%
...
%    the data length is smaller than 5000, else N=4000.
%
...
%
% $Date: 2021/11/22 12:33:10 $
% $Revision: 5.18 $
%
% $Log: fnn.m,v $
% Revision 5.18  2021/11/22 12:33:10  marwan
% minor code polishing in fnn
%
% Revision 5.17  2021/11/04 15:34:31  marwan
% fnn corrected
%
% Revision 5.16  2021/11/02 08:30:04  marwan
...
case 'init'
    ...
    h2=textwrap(h,{[char(169),' PIK 2021'],'Potsdam Institute for','Climate Impact Research'});
    set(h,'String',h2)
    ...
    h=uicontrol(props.edit,...
            'Tag','r',...
	        'String',num2str(r),...
	        'ToolTip','Criterion value (suggested: 10).',...
    ...
    h=uicontrol(props.edit,...
            'Tag','s',...
	        'String',num2str(s),...
	        'ToolTip','Tolerance, Eq. (5) in Kennel (suggested: 2)',...
...
case 'compute'
    ...
    % sub-sample (if specified)
    if maxN >= length(x)
        idx = 1:length(x);
    else
        idx = randperm(length(x),maxN); %random samples
        x = x(idx,:);
    end
    ...
    for m = 2:maxM+1
        ...
        switch method
           case 1
              D2 = squareform(pdist(x1,'chebychev').^2); % maximum norm
           case 2
              D2 = squareform(pdist(x1,'squaredeuclidean')); % euclidean norm
           case 3
              D2 = squareform(pdist(x1,'cityblock').^2); % minimum norm
        end
        D2(end-t+1:end,:) = []; % remove last part due to embedding
        D2(:,end-t+1:end) = []; % remove last part due to embedding


        % nearest neigbours
        D2 = D2 + eye(size(D2)) * 1e16; % fill diagonal with big numbers to avoid self-comparison
        [minD nn] = min(D2); minD = minD';
        
        % create phase space vector
        x2 = embed(x,m,t);
        switch method
           case 1
              Dm2 = max(abs(x2 - x2(nn,:)),[],2).^2; % distances with nearest neighbours in higher dimension
           case 2
              Dm2 = sum((x2 - x2(nn,:)).^2,2); % distances with nearest neighbours in higher dimension
           case 3
              Dm2 = sum(x2 - x2(nn,:),2).^2; % distances with nearest neighbours in higher dimension
        end
        increase = (Dm2 - minD);
        ratio = increase ./ minD;
        condition4 = sqrt(ratio) > r; % Kennel, Eq. (4)
        condition5 = sqrt(increase) / R_A > s; % Kennel, Eq. (5)
        FNN(m-1,1) = sum(condition4 | condition5) / numel(ratio);
