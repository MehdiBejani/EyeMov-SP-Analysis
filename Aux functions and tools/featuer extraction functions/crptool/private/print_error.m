...
%
% $Date: 2019/03/22 14:44:18 $
% $Revision: 4.12 $
%
% $Log: print_error.m,v $
% Revision 4.12  2019/03/22 14:44:18  marwan
...
v = ver(tb_path);
  ...
  if ~strcmpi(lasterr,'Interrupt')
    ...
    disp('----------------------------');
    if ~isempty(findstr(x,'Out of memory')) || ~isempty(findstr(x,'array exceeds maximum array size'))
        disp('   Your computer has probably not enough memory to analyse')
        disp('   your data. You can find help on this problem at')
        disp('   * http://tocsy.pik-potsdam.de/CRPtoolbox/content_faq.php#length')
        disp('   * http://tocsy.pik-potsdam.de/CRPtoolbox/content_faq.php#long_data')
    else
