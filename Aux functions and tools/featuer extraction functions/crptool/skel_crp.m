function X_new = skel_crp(varargin)
% SKELCRP  Removes thickening of diagonal lines in RP.
% 
%    Y = SKEL_CRP(X) creates a new recurrence matrix Y in which
%    all recurrence points of the recurrence matrix X  are removed
%    which lead to a thickening of diagonal lines. Slubs, but also 
%    block structures are removed in favour of the longest diagonal 
%    lines (skeletonization). Whenever a diagonal line (starting 
%    with the longest lines contained in the diagonal line length 
%    histogram) encounters an adjacent diagonal line, this adjacent 
%    line and - recursively - all its consecutive adjacent lines, 
%    get deleted. 
%
%    Examples: a = sin(linspace(0,5*2*pi,100));
...
%
% $Log: skel_crp.m,v $
% Revision 5.2  2021/11/22 16:42:48  marwan
