function w = sp_gaussianwin(n,r,sig2)
...
elseif r >= 1
    ...
    if 0 % there is no effect of sigma on this part
        sig2=.1;
        tmp1=(pi/per*(t(1:tl) - per)); tmp2=(pi/per*(t(th:end) - 1 + per));
        mul1=1/(exp(-tmp1(end).^2)/sig2); mul2= 1/(exp(-tmp2(1).^2)/sig2);
        w = [mul1*(exp(-tmp1.^2)/sig2);  ones(th-tl-1,1); mul2*(exp(-tmp2.^2)/sig2)];
    else
        %sig2=pi;
        tmp1=(1/sig2*(t(1:tl) - per)); tmp2=(1/sig2*(t(th:end) - 1 + per));
        mul1=1/(exp(-tmp1(end).^2)); mul2= 1/(exp(-tmp2(1).^2));
        w = [mul1*(exp(-tmp1.^2));  ones(th-tl-1,1); mul2*(exp(-tmp2.^2))];
    end
end 
